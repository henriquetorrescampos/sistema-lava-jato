...
function initReviewsPage() {
  ...
  if (!reviewsTitle || !avgStars || !totalLabel || !reviewsList || !toggleFormButton || !reviewForm) return;
  ...
  const initialReviews = [
    {
      reviewer: 'Mariana P.',
      rating: 5,
      comment: 'Atendimento excelente e lavagem impecavel. Meu carro saiu com aspecto de novo.',
      photoUrl: 'https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=700&q=80',
      createdAt: '2026-03-28T12:00:00.000Z'
    },
    {
      reviewer: 'Rafael M.',
      rating: 4,
      comment: 'Gostei muito da higienizacao interna, so atrasou 10 minutos no horario combinado.',
      photoUrl: '',
    ...
    },
    {
      reviewer: 'Camila S.',
      rating: 5,
      comment: 'Polimento com otimo resultado, equipe atenciosa e ambiente organizado.',
      photoUrl: '',
  ...
  function renderReviews() {
    const reviews = readStoredReviews().slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    ...
    if (!reviews.length) {
      reviewsList.innerHTML = '<article class="review-card"><p>Este estabelecimento ainda nao possui comentarios.</p></article>';
      return;
    }
    ...
    reviewsList.innerHTML = reviews.map((review) => {
      ...
      const rating = Number(review.rating || 0).toFixed(1).replace('.', ',');
...
function initAuthPage() {
  ...
  if (!signupForm || !loginForm || !googleSignupButton || !googleLoginButton || !cpfInput || !phoneInput || !switchLogin || !switchSignup || !loginPanel || !signupPanel || !authTitle || !authSubtitle) return;
  ...
  function setMode(mode) {
    ...
    if (isLogin) {
      ...
      authSubtitle.textContent = 'Voce pode ver os lava jatos sem login, mas para agendar e avaliar e necessario entrar na conta.';
    } else {
      authTitle.textContent = 'Crie sua conta para usar o app';
      authSubtitle.textContent = 'Cadastro rapido para pesquisar lava jatos, agendar servicos e avaliar estabelecimentos.';
    }
  }
  ...
  function upsertGoogleUser(name, email) {
    ...
    setCurrentUser({ name: payload.name, email: payload.email, authProvider: 'google' });
  }
  ...
  function handleAuthQueryFeedback() {
    ...
    if (auth === 'google_not_configured') {
      alert('Login Google ainda nao configurado no servidor. Preencha GOOGLE_CLIENT_ID e GOOGLE_CLIENT_SECRET no arquivo .env.');
      window.history.replaceState({}, '', 'cadastro.html');
      return;
    }
  ...
  loginForm.addEventListener('submit', (event) => {
    ...
    const users = getUsersFromStorage();
    const user = users.find((item) => item.email === email && item.authProvider === 'email');
    if (!user || user.password !== password) {
      alert('Email ou senha invalidos.');
      return;
    }

    setCurrentUser({ name: user.name, email: user.email, authProvider: user.authProvider });
  ...
  signupForm.addEventListener('submit', (event) => {
    ...
    const users = getUsersFromStorage();
    const alreadyExists = users.some((user) => user.email === email || user.cpf === cpf);
    if (alreadyExists) {
      alert('Ja existe um cadastro com este email ou CPF.');
      return;
    }
    ...
    saveUsersToStorage(users);
    setCurrentUser({ name, email, authProvider: 'email' });
